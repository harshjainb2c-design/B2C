...
export const ProductSort = memo(({ value, onChange }: ProductSortProps) => {
  return (
    <div className="flex items-center gap-2 select-none font-inter">
      <label htmlFor="sort" className="text-xs font-inter font-medium uppercase tracking-wider text-neutral-400 hidden sm:block">
        Sort:
      </label>
      <select
        id="sort"
        ...
        className="px-3.5 py-2 rounded-full text-xs border border-white/15 bg-neutral-900/80 text-white focus:outline-none focus:border-white/40 cursor-pointer font-inter font-medium"
      >
